...
// Resets tracking for a new attempt. Relies on the caller (wait_for_submit())
// having already confirmed every button is physically released via
// wait_release_all() -- so there is nothing currently held to snapshot, and
// buttons_poll()'s normal debounced released->pressed transition is enough to
// catch every dot the user presses next. Deliberately does NOT read the raw
// pins here: an un-debounced read right at reset can catch mechanical
// contact bounce from the release that just happened, planting a phantom bit
// in the mask that makes every subsequent comparison wrong.
...
// six dot buttons behave for any existing caller.
// ---------------------------------------------------------------------------

static bool g_submit_raw = false, g_submit_stable = false;
static uint32_t g_submit_last_change_ms = 0;

static void submit_begin() {
  pinMode(PIN_SUBMIT, INPUT_PULLUP);
  g_submit_raw = g_submit_stable = false;
}

// Edge-triggered: returns true exactly once per physical press, on the
// debounced LOW-going transition. Call it repeatedly from a polling loop.
static bool submit_pressed() {
  bool r = (digitalRead(PIN_SUBMIT) == LOW);   // active LOW, same convention as the dot buttons
  uint32_t now = millis();
  if (r != g_submit_raw) { g_submit_raw = r; g_submit_last_change_ms = now; }
  if (r != g_submit_stable && (now - g_submit_last_change_ms) >= BUTTON_DEBOUNCE_MS) {
    g_submit_stable = r;
    return r;
  }
  return false;
