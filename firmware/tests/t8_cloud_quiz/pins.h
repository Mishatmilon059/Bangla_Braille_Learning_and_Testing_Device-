...
// --- submit button -----------------------------------------------------
// GPIO 12 is a strapping pin that must read LOW at boot or the chip picks
// the wrong flash voltage -- see the note below. A plain switch to GND is
// safe here specifically because it can only ever pull the pin LOW or leave
// it floating to the chip's own internal default (also LOW); a button can
// never force it HIGH, so it can't disturb the boot-time strap. This is why
// GPIO 12 is the one spare pin usable for a new button, where GPIO 0 (the
// board's existing BOOT button) is not: reusing GPIO 0 risks accidentally
// forcing the chip into flashing mode on a reset.
static const int PIN_SUBMIT = 12;
