...
// Reports one attempt to Supabase's `attempts` table using the REAL features
// this sketch already computed (accuracy, mastery, streaks) and the ML
// model's decision as the authoritative teaching_action/confidence_state --
// matching how braille_tutor.ino prefers the model over the rule engine when
// it's available. Without this, none of this sketch's per-attempt data ever
// left the board; it only ever went to Serial.
...
void setup() {
  ...
  snprintf(g_session_id, sizeof(g_session_id), "esp32_mltest_%lu", millis());
