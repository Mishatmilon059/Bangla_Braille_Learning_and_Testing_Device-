// ============================================================
//  p4 — BUTTONS ONLY: chord input decoded to the serial log
// ============================================================
// Proves: the seven-switch input path. Six dot buttons toggle,
// Submit decodes the held chord into a Bangla letter, Enter
// prints and clears the accumulated message. No audio, no motors.
//
// Use: tap dots (each tap toggles that dot on/off), press Submit
// to decode, press Enter to print the message. Submitting an
// empty cell inserts a space.
//
// Two-cell characters use a dot-5 prefix, matching p5/p6:
//   ঋ = Submit dot 5, then Submit dots 1,2,3,5
//   ৎ = Submit dot 5, then Submit dots 2,3,4,5
//
...
//   enter  GPIO 23
//
// NOTE: GPIO 23 is the DFPlayer TX line in p5/p6, so the Enter
// button must move before this wiring can coexist with audio.
// ============================================================
...
void setup() {
  ...
  Serial.println("- For ৎ: Submit Dot 5, then Submit Dots 2,3,4,5.");
...
void decodeAndPrintChord() {
  ...
  if (pendingPrefix != 0) {
    ...
    // Evaluate combinations based on the active prefix
    if (pendingPrefix == 0b010000) { // If the prefix was Dot 5
      if (typedPattern == 0b010111) {
        // Dot 5 followed by Dots 1,2,3,5
        foundLetter = "ঋ";
      } 
      else if (typedPattern == 0b011110) {
        // Dot 5 followed by Dots 2,3,4,5
        foundLetter = "ৎ";
      }
    }
  ...
  if (typedPattern == 0b010000) { // Dot 5 only
    ...
    Serial.println("Prefix (Dot 5) entered. Submit 2nd cell (1,2,3,5 for 'ঋ' OR 2,3,4,5 for 'ৎ')...");
    Serial.flush();
    resetChord();
    return;
  } 
