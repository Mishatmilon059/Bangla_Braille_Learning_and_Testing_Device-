...
//   submit      GPIO 18
//   enter       GPIO 16   (has a usable internal pull-up, unlike GPIO 34)
//                         NB: on an ESP32-WROVER, GPIO 16/17 are wired to
//                         PSRAM and cannot be used. WROOM boards only.
//   motors      GPIO 13, 14, 26, 27, 32, 33
//   DFPlayer RX GPIO 25 / TX GPIO 23 (1k in series on DFPlayer RX)
//
// Audio:
//   Tracks are addressed with playMp3Folder(), so the card root must hold
//   mp3/0001.mp3 .. mp3/0060.mp3 -- copy the CONTENTS of sd_card/, not the
//   sd_card folder itself. FAT32 only.
//   The speaker goes across SPK_1 and SPK_2. Neither leg touches GND;
//   grounding one is silent and can kill the amp.
//   Keep begin()'s isACK argument true. With it false the library returns
//   `|| !isACK` and reports success even with nothing plugged in, so a dead
//   module looks identical to a working one.
// ============================================================
...
void setup() {
  ...
  if (!dfPlayer.begin(mySoftwareSerial, true, true)) {
    ...
    dfPlayer.volume(22);   // 0..30; 12 is very quiet on a small speaker 
  }
}
