...
//
// Ported from web/storage.js -- behavior is intentionally byte-identical.
...
import { SUPABASE_URL, SUPABASE_ANON_KEY, DEVICE_ID_KEY } from './config';
import { ALL_FEATURE_RANGES, MASTERY_INITIAL, updateMastery } from './ruleEngine';
...
export class AttemptLogger {
  ...
  async syncRemoteForUser(userId, learner) {
    if (!this.configured || (typeof navigator !== 'undefined' && !navigator.onLine)) return;
    try {
